...
import { getQuestionnaireByName } from './questionnaire';
...
describe('Questionnaire utilities', () => {
  ...
  describe('getQuestionnaireByName', () => {
    ...
    it('should throw error when questionnaire not found', async () => {
      await expect(getQuestionnaireByName(medplum, 'non-existent-questionnaire')).rejects.toThrow(
        'Questionnaire with name "non-existent-questionnaire" not found'
      );
    });
  });
});
